...
const CourseDetails = () => {
      ...
      try {
        const fetchedCourses = await Promise.all(
          courseIds.map(async (courseId) => {
            const response = await axios.get(`https://astra-ul2e.onrender.com/courses/${courseId}`);
  ...
  const handleDelete = async (courseId) => {
    ...
    try {
      const response = await axios.delete(`https://astra-ul2e.onrender.com/getSelectedCourses/${userEmail}/${courseId}`);
